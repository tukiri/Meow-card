...
//         String patternType = node.getAttribute('patternType').toString(),
//             rgb;
...
//       List image = [];
//
//       var drawingFile = _excel._archive.findFile("xl/" +
//           target.toString().replaceAll("worksheets", "worksheets/_rels") +
//           ".rels");
//       drawingFile!.decompress();
//
//       var drawingContent = XmlDocument.parse(utf8.decode(drawingFile.content));
...
//
//
//       var imageFile = _excel._archive.findFile(
//           drawingPath.replaceAll("drawings/", "drawings/_rels/") + ".rels");
//       imageFile!.decompress();
//
//       var imageFileContent = XmlDocument.parse(utf8.decode(imageFile.content));
//       List imagePathList = imageFileContent
//           .findElements('Relationships')
//           .first
//           .findElements('Relationship')
//           .toList();
...
//         var imgUrl = element.getAttribute('Target').toString().replaceAll(
//             "../", "xl/");
//         image.add({"id": imgId, "url": imgUrl});
//       });
//
//       for (var i in image) {
//         var imgFile = _excel._archive.findFile(i["url"]);
//         imgFile!.decompress();
//         i["img"] = imgFile.content;
//       }
//
//       var imageConfig = _excel._archive.findFile(drawingPath);
//       imageConfig!.decompress();
//
//       var imageConfigContent = XmlDocument.parse(
//           utf8.decode(imageConfig.content));
//
//       ExcelColumn(n) {
//         var ab = '_ABCDEFGHIJKLMNOPQRSTUVWXYZ';
//         var out = "";
//
//
//         while (n >= 26) {
...
//       };
//
//       List imageConfigList = imageConfigContent
//           .findElements('xdr:wsDr')
//           .first
//           .findElements("xdr:twoCellAnchor")
//           .toList();
//
//
//       imageConfigList.forEach((e) {
//         var eList = {};
//
//         var eFrom = e
//             .getElement("xdr:from");
...
//             .getElement("xdr:to");
//
//         eList["from"] = {"row":eFrom
//             .getElement("xdr:row").text,"col":eFrom
//             .getElement("xdr:col").text};
//
//         eList["to"] = {"row":eTo
//             .getElement("xdr:row").text,"col":eTo
//             .getElement("xdr:col").text};
//
//
//         var imgFrom = ExcelColumn(int.parse(eList["from"]["col"])) + eList["from"]["row"];
//         var imgTo = ExcelColumn(int.parse(eList["to"]["col"])) + eList["to"]["row"];
//
//         var eId = e
//             .getElement("xdr:pic")
//             .getElement("xdr:blipFill")
//             .getElement("a:blip")
...
//         });
//       });
//       sheetObject._sheetImg = image;
//     }else{
//       sheetObject._sheetImg = [];
//     }
//
//
...
//             .value(int.parse(_parseValue(node
//             .findElements('v')
//             .first)));
//         break;
//     // boolean
//       case 'b':
//         value = _parseValue(node
//             .findElements('v')
//             .first) == '1';
//         break;
//     // error
...
//         if (false) {
//           value = Formula.custom(_parseValue(valueNode.first).toString());
//         } else {
//           if (s1 != null) {
...
//               value =
//               '${_twoDigits(date.hour)}:${_twoDigits(date.minute)}:${_twoDigits(
//                   date.second)}';
//               // number
//             } else {
